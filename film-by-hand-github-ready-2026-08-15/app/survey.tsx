...
import { ageRanges, citiesByCountry, countries, difficultyLabels, equipment, filmTypes, frequency, initialAnswers, knowledge, photographyRelationships, problems, stages, type Answers } from "./survey-data";
...
function SearchableSelect({ label, value, options, placeholder, disabled, allowCustom, onSelect }: { label: string; value: string; options: Array<{ label: string; value: string }>; placeholder: string; disabled?: boolean; allowCustom?: boolean; onSelect: (value: string, label: string) => void }) {
  const [query, setQuery] = useState(value);
  const [open, setOpen] = useState(false);
  const [active, setActive] = useState(0);
  const matches = options.filter((option) => option.label.toLowerCase().includes(query.toLowerCase())).slice(0, 80);
  const shown = allowCustom && query.trim() && !matches.some((option) => option.label.toLowerCase() === query.trim().toLowerCase()) ? [...matches, { label: `Use “${query.trim()}”`, value: query.trim() }] : matches;
  const choose = (option: { label: string; value: string }) => { const display = option.label.startsWith("Use “") ? option.value : option.label; onSelect(option.value, display); setQuery(display); setOpen(false); setActive(0); };
  return <div className={`combobox ${disabled ? "disabled" : ""}`}><label>{label}<input role="combobox" aria-expanded={open} aria-controls={`${label.replace(/\W/g, "-")}-listbox`} aria-autocomplete="list" disabled={disabled} value={query} placeholder={placeholder} onFocus={() => setOpen(true)} onChange={(e) => { setQuery(e.target.value); setOpen(true); setActive(0); }} onKeyDown={(e) => { if (e.key === "ArrowDown") { e.preventDefault(); setOpen(true); setActive((i) => Math.min(i + 1, shown.length - 1)); } if (e.key === "ArrowUp") { e.preventDefault(); setActive((i) => Math.max(i - 1, 0)); } if (e.key === "Enter" && open && shown[active]) { e.preventDefault(); choose(shown[active]); } if (e.key === "Escape") setOpen(false); }} /></label>{open && !disabled && <ul role="listbox" id={`${label.replace(/\W/g, "-")}-listbox`}>{shown.length ? shown.map((option, index) => <li role="option" tabIndex={-1} aria-selected={index === active} className={index === active ? "active" : ""} key={`${option.value}-${index}`} onMouseDown={(e) => e.preventDefault()} onClick={() => choose(option)} onKeyDown={(e) => { if (e.key === "Enter") choose(option); }}>{option.label}</li>) : <li className="empty">No matches. Keep typing to enter your city.</li>}</ul>}</div>;
}
...
export function Survey() {
  ...
  const titles = ["What’s your name?", "What country are you based in?", "What city are you based in?", "What is your age range?", "How would you describe your relationship with photography?", "When did you start developing film?", "What do you currently use to develop film?", "Which film stocks, brands or types do you use?", "How do you decide on chemistry, time and agitation?", "How difficult is each stage?", "How often do you encounter problems?", "What problems have you experienced?", "Are there any parts of your film development process that you find frustrating?", "How large is your film-processing workspace?"];
  const isMultiSelect = [7, 8, 11].includes(step);
  const cityOptions = (citiesByCountry[answers.countryCode] ?? []).map((city) => ({ label: city, value: city }));
  return <main className="survey-shell"><header className="survey-header"><button className="wordmark" onClick={() => setStarted(false)}>35 / HAND</button><div className="progress-meta"><span>{String(step + 1).padStart(2, "0")} / {TOTAL}</span><span>{Math.round(((step + 1) / TOTAL) * 100)}%</span></div><div className="progress-track"><span style={{ width: `${((step + 1) / TOTAL) * 100}%` }} /></div></header><section className="question" key={step}><p className="question-index">QUESTION {String(step + 1).padStart(2, "0")} <i>Required</i></p><h2 ref={headingRef} tabIndex={-1} aria-describedby={isMultiSelect ? "multi-select-help" : undefined}>{titles[step]}</h2>{isMultiSelect && <p className="hint question-hint" id="multi-select-help">Select all that apply.</p>}<div className="answer-area">
    {step === 0 && <input className="identity-input" aria-label="Your name" placeholder="Your name" autoComplete="name" value={answers.name} onChange={(e) => set("name", e.target.value)} />}
    {step === 1 && <SearchableSelect label="Country" value={answers.country} options={countries.map((country) => ({ label: country.name, value: country.code }))} placeholder="Search countries" onSelect={(code, name) => setAnswers((a) => ({ ...a, country: name, countryCode: code, city: "" }))} />}
    {step === 2 && <SearchableSelect label="City" value={answers.city} options={cityOptions} placeholder={answers.country ? "Search or type your city" : "Choose a country first"} disabled={!answers.country} allowCustom onSelect={(_, city) => set("city", city)} />}
    {step === 3 && <div className="choices">{ageRanges.map((o) => <Choice key={o} label={o} selected={answers.ageRange === o} onClick={() => set("ageRange", o)} />)}</div>}
    {step === 4 && <div className="choices relationship-choices">{photographyRelationships.map((o) => <Choice key={o} label={o} selected={answers.photographyRelationship === o} onClick={() => set("photographyRelationship", o)} />)}</div>}
    {step === 5 && <div className="year-picker"><div className="year-display"><strong>{answers.startYear}</strong><span>{Math.floor(answers.startYear / 10) * 10}s</span></div><input aria-label="Year you started developing film" type="range" min="1920" max="2026" value={answers.startYear} onChange={(e) => set("startYear", Number(e.target.value))} /><div className="range-labels"><span>1920</span><span>2026</span></div></div>}
    {step === 6 && <><div className="choices">{equipment.map((o) => <Choice key={o} label={o} selected={answers.equipment === o} onClick={() => set("equipment", o)} />)}</div>{answers.equipment === "Other" && <input className="other-input" placeholder="Tell us what you use" value={answers.equipmentOther} onChange={(e) => set("equipmentOther", e.target.value)} />}</>}
    {step === 7 && <><MultiChoices options={filmTypes} values={answers.filmTypes} onChange={(v) => set("filmTypes", v)} />{answers.filmTypes.includes("Other") && <input className="other-input" placeholder="Which other process?" value={answers.filmTypesOther} onChange={(e) => set("filmTypesOther", e.target.value)} />}</>}
    {step === 8 && <><MultiChoices options={knowledge} values={answers.knowledgeSources} onChange={(v) => set("knowledgeSources", v)} />{answers.knowledgeSources.includes("Other") && <input className="other-input" placeholder="Which other source?" value={answers.knowledgeOther} onChange={(e) => set("knowledgeOther", e.target.value)} />}</>}
    {step === 9 && <div className="matrix"><div className="matrix-key">1 — Very easy <span>5 — Very difficult</span></div>{stages.map((stage, idx) => <fieldset key={stage}><legend><span>{String(idx + 1).padStart(2, "0")}</span>{stage}</legend><div className="rating">{[1,2,3,4,5].map((n) => <label key={n} className={answers.difficulty[stage] === n ? "active" : ""}><input type="radio" name={stage} value={n} checked={answers.difficulty[stage] === n} onChange={() => set("difficulty", { ...answers.difficulty, [stage]: n })} /><b>{n}</b><small>{difficultyLabels[n - 1]}</small></label>)}</div></fieldset>)}</div>}
    {step === 10 && <div className="choices">{frequency.map((o) => <Choice key={o} label={o} selected={answers.problemFrequency === o} onClick={() => set("problemFrequency", o)} />)}</div>}
    {step === 11 && <><MultiChoices options={problems} values={answers.problems} onChange={(v) => set("problems", v)} />{answers.problems.includes("Other") && <input className="other-input" placeholder="Which other problem?" value={answers.problemsOther} onChange={(e) => set("problemsOther", e.target.value)} />}</>}
    {step === 12 && <><Choice label="Nothing in particular" selected={answers.frustration === "Nothing in particular"} onClick={() => set("frustration", "Nothing in particular")} /><textarea aria-label="Parts of the film development process you find frustrating" className="reflection" rows={6} maxLength={1500} placeholder="If anything comes to mind, describe it here…" value={answers.frustration === "Nothing in particular" ? "" : answers.frustration} onChange={(e) => set("frustration", e.target.value)} /><div className="char-count">{answers.frustration === "Nothing in particular" ? 0 : answers.frustration.length} / 1500</div></>}
    {step === 13 && <div className="workspace-picker"><div className="workspace-value" aria-live="polite"><strong>{Number.isInteger(answers.workspaceAreaM2) ? answers.workspaceAreaM2 : answers.workspaceAreaM2.toFixed(1)} <small>m²</small></strong><span>{workspaceFt2} ft²</span></div><p className="workspace-note" id="workspace-help">Estimate the total floor area you have available. The references below are approximate footprints.</p><div className="scale-scene" aria-hidden="true">{scaleReferences.map((reference) => <div key={reference.name} className={`scale-reference ${reference.className} ${activeScale.name === reference.name ? "active" : ""}`}><div className="silhouette"><i /><i /><i /></div><span>{reference.name}</span><small>~{reference.area} m²</small></div>)}</div><input className="workspace-range" aria-label="Available film-processing workspace area in square metres" aria-describedby="workspace-help" aria-valuemin={0.5} aria-valuemax={120} aria-valuenow={answers.workspaceAreaM2} aria-valuetext={workspaceLabel} type="range" min="0.5" max="120" step="0.5" value={answers.workspaceAreaM2} onChange={(e) => setWorkspaceArea(Number(e.target.value))} /><div className="range-labels"><span>0.5 m²</span><span>120 m²</span></div></div>}
